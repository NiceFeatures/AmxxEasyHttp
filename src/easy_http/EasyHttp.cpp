...
namespace
{
    ...
    int OnFtpWildcardChunkBegin(const curl_fileinfo *file_info, void *user_data, int /*remaining_files*/)
    {
        ...
        // Security: Ensure filename doesn't contain path separators to prevent traversal
        ...
        context->current_file_path = context->destination_directory / filename;
...
EasyHttp::~EasyHttp()
{
    ...
    while (true)
    {
        ...
        std::lock_guard<std::mutex> lock_guard(completed_requests_mutex_);
...
void EasyHttp::WorkerLoop()
{
    while (true)
    {
        ...
        {
            std::unique_lock<std::mutex> lock_guard(pending_requests_mutex_);
            pending_requests_cv_.wait(lock_guard, [this]()
                                      { return stop_requested_ || !pending_requests_.empty(); });

            if (pending_requests_.empty() && stop_requested_)
                return;
