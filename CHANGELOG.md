...
### Fixed
- **File I/O**: Fixed `HttpDownload` silently failing to write files to disk. It now correctly implements `std::ofstream` with a `WriteCallback`.
- **Linux Compatibility**: Added path normalization in `ValidatePath` (converts `\` to `/`) to resolve issues where the filesystem treated paths as single file names instead of directories.
- **Auto Updater Loop**: Corrected the module update logic (`.new` files swap) which previously caused infinite loops due to failed writes and path issues.
...
### Fixed
- **Compatibility**: Switched build environment to Ubuntu 16.04 (GLIBC 2.23) to ensure support for older Linux server distributions.
